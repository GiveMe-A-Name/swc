...
impl Lexer<'_> {
    ...
    pub(super) fn read_number_literal(&self, source: &str, span: Span) -> LexResult<f64> {
        let value = if let Some(s) = source
            .strip_prefix("0x")
            ...
            .or_else(|| source.strip_prefix("0B"))
        {
            ...
            usize::from_str_radix(source, 8)
                .map_err(|_| return Error::new(span, SyntaxError::LegacyDecimal))?
                as f64
        } else {
            // TODO: panic -> return a Error,
            source.parse::<f64>().unwrap_or_else(|_| {
                panic!("failed to parse number: {}", source);
            })
        };
        Ok(value)
    }
